...
const Payments = () => {
  ...
  useEffect(() => { setCurrentBalance(Number(profile?.balance || 0)); }, [profile?.balance, forceUpdate]);

  useEffect(() => {
    if (!user) return;
    Promise.all([
      db.getPayments(user.id),
      db.getRequisites(user.id),
    ]).then(([txRes, reqRes]) => {
  ...
  const handleWithdrawRequest = async () => {
    if (!user || !withdrawAmount || !withdrawMethod) { toast({ title: "Укажите сумму и метод вывода", variant: "destructive" }); return; }
    ...
    if (!methodReq || !methodReq.details) { toast({ title: "Сначала заполните реквизиты для этого метода", variant: "destructive" }); return; }
    setRequesting(true);
    try {
      const newBalance = currentBalance - amount;
      await db.updateProfile(user.id, { balance: newBalance });
      await db.createPayment({ user_id: user.id, amount, type: "withdrawal", status: "pending", description: `Вывод (${methodLabels[withdrawMethod]})`, payment_method: withdrawMethod });
      await db.createLog({ user_id: user.id, event: "withdrawal_request", details: `Сумма: ${amount} ₽, Метод: ${methodLabels[withdrawMethod]}` });
      if (profile) profile.balance = newBalance;
      ...
      const { data } = await db.getPayments(user.id);
  ...
  const validateRequisite = (method: string, value: string): string | null => {
    ...
    if (!v) return null;
    switch (method) {
      case "card": { const digits = v.replace(/[\s\-]/g, ""); if (!/^\d{13,19}$/.test(digits)) return "Номер карты должен содержать 13–19 цифр"; return null; }
      case "sbp": { const phone = v.replace(/[\s\-\(\)]/g, ""); if (!/^\+?[0-9]{10,15}$/.test(phone)) return "Введите корректный номер телефона"; return null; }
      case "crypto": { if (v.length < 20 || v.length > 128) return "Адрес кошелька должен быть от 20 до 128 символов"; if (!/^[a-zA-Z0-9\-_:\.]+$/.test(v)) return "Адрес содержит недопустимые символы"; return null; }
      case "lolzteam": { if (v.length < 2 || v.length > 50) return "Ник должен быть от 2 до 50 символов"; return null; }
      default: return null;
    }
  };
  ...
  const saveRequisites = async () => {
    ...
    for (const method of Object.keys(reqValues)) {
      if (reqValues[method]) { const err = validateRequisite(method, reqValues[method]); errors[method] = err; if (err) hasError = true; }
    }
    setReqErrors(errors);
    if (hasError) { toast({ title: "Проверьте правильность данных", variant: "destructive" }); return; }
    try {
      for (const method of Object.keys(reqValues)) {
        if (reqValues[method]) { await db.upsertRequisite({ user_id: user.id, method, details: reqValues[method].trim() }); }
      }
      const { data } = await db.getRequisites(user.id);
      setRequisites(data as any[] || []);
      setEditingRequisites(false); setReqErrors({});
  ...
  return (
    <DashboardLayout>
      <div className="mb-6"><h1 className="text-2xl font-display font-bold mb-1">Выплаты</h1><p className="text-sm text-muted-foreground">Управление выплатами и реквизитами</p></div>
      <div className="grid sm:grid-cols-3 gap-3 mb-8">
        {[
          ...
          <motion.div key={s.label} initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="bg-card border border-border rounded-2xl p-5">
            <div className="flex items-center gap-3 mb-2"><div className="w-10 h-10 rounded-xl bg-primary/8 flex items-center justify-center"><s.icon className={`h-5 w-5 ${s.color}`} /></div></div>
      ...
      <motion.div initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} className="bg-card border border-border rounded-2xl p-6 mb-6">
        <div className="flex items-center justify-between mb-4">
          ...
          <Button variant="outline" size="sm" className="rounded-xl h-8 text-xs" onClick={() => setEditingRequisites(!editingRequisites)}><Settings2 className="h-3.5 w-3.5 mr-1" /> {editingRequisites ? "Отмена" : "Настроить"}</Button>
        </div>
        {editingRequisites ? (
          ...
          <div className="grid sm:grid-cols-2 gap-2">
            {Object.entries(methodLabels).map(([key, label]) => {
              ...
              return (<div key={key} className="flex items-center justify-between p-3 rounded-xl bg-muted/50"><span className="text-sm font-medium">{label}</span><span className="text-sm text-muted-foreground font-mono">{req?.details || "—"}</span></div>);
            })}
          </div>
        )}
      </motion.div>
      ...
      <div className="bg-card border border-border rounded-2xl p-6 mb-6">
        ...
        <div className="flex flex-wrap gap-3">
          ...
          <Select value={withdrawMethod} onValueChange={setWithdrawMethod}>
            ...
            <SelectContent>{Object.entries(methodLabels).map(([key, label]) => (<SelectItem key={key} value={key}>{label}</SelectItem>))}</SelectContent>
          </Select>
          <Button className="gradient-primary text-primary-foreground border-0 rounded-xl" onClick={handleWithdrawRequest} disabled={requesting}>{requesting ? "Отправка..." : "Запросить"}</Button>
        </div>
      </div>
              ...
              <motion.div key={tx.id} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: i * 0.02 }}
                ...
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <div className="text-xs text-muted-foreground">{new Date(tx.created_at).toLocaleDateString("ru-RU")}{tx.payment_method && ` · ${methodLabels[tx.payment_method] || tx.payment_method}`}</div>
                  </div>
                </div>
                <div className="flex items-center gap-3">
                  <span className={`text-sm font-mono font-semibold ${tx.type !== "withdrawal" ? "text-success" : "text-destructive"}`}>{tx.type !== "withdrawal" ? "+" : "-"}{Number(tx.amount).toLocaleString("ru-RU")} ₽</span>
                  <Badge variant={tx.status === "completed" || tx.status === "approved" ? "default" : "outline"} className="text-[11px]">{tx.status === "completed" || tx.status === "approved" ? "Завершено" : tx.status === "pending" ? "Ожидание" : "Отклонено"}</Badge>
                </div>
              </motion.div>
            ))}
          </div>
