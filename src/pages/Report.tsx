...
const Report = () => {
  ...
  useEffect(() => {
    const fetchTasks = async () => {
      if (!user) return;
      const { data: tasks } = await db.getUserTasks(user.id, ["assigned", "in_progress"]);
      const { data: existingReports } = await db.getReports(user.id);
      const submitted = new Set(((existingReports as any[]) || []).map((r: any) => r.user_task_id));
      setSubmittedTaskIds(submitted);
      setUserTasks(((tasks as any[]) || []).filter((t: any) => !submitted.has(t.id)));
      setLoading(false);
    };
    fetchTasks();
  }, [user]);
  ...
  const handleSubmit = async () => {
    ...
    setSubmitting(true);
    try {
      const { error } = await db.createReport({ user_task_id: selectedTask, user_id: user.id, format, content: format === "text" ? textReport : `${format} отчёт`, status: "pending" });
      if (error) throw error;
      await db.updateUserTask(selectedTask, { status: "submitted" });
      await db.createLog({ user_id: user.id, event: "report_submitted", details: `Task: ${selectedTask}, Format: ${format}` });
      toast({ title: "Отчёт отправлен!" });
      setSubmitted(true);
    } catch (error: any) { toast({ title: "Ошибка", description: error?.message, variant: "destructive" }); }
  ...
  if (selectedTask === null) return (
    <DashboardLayout>
      <div className="mb-6"><h1 className="text-2xl font-display font-bold mb-1">Отправить отчёт</h1><p className="text-sm text-muted-foreground">Выберите задание (1 отчёт на задание)</p></div>
      {loading ? <div className="text-center py-12 text-muted-foreground">Загрузка...</div> :
       userTasks.length === 0 ? (<div className="bg-card border border-border rounded-2xl p-12 text-center"><AlertTriangle className="h-10 w-10 text-muted-foreground mx-auto mb-3" /><p className="text-muted-foreground">Нет заданий для отчёта.</p></div>) : (
        <div className="space-y-2">
          {userTasks.map((ut, i) => (
            <motion.div key={ut.id} initial={{ opacity: 0, y: 10 }} animate={{ opacity: 1, y: 0 }} transition={{ delay: i * 0.03 }}
              ...
              <div className="flex-1 min-w-0"><span className="font-medium text-sm group-hover:text-primary transition-colors truncate block">{ut.tasks?.title}</span><Badge variant="outline" className="text-[11px] mt-1">{ut.tasks?.task_categories?.name}</Badge></div>
              <div className="flex items-center gap-3 ml-4 shrink-0"><span className="text-sm font-mono font-bold">{ut.tasks?.reward ? `${Number(ut.tasks.reward).toLocaleString("ru-RU")} ₽` : ""}</span><ChevronRight className="h-4 w-4 text-muted-foreground/40" /></div>
            </motion.div>
          ))}
        </div>
      )}
  ...
  return (
    <DashboardLayout>
      ...
      <div className="max-w-2xl space-y-4">
        <div className="bg-card border border-border rounded-2xl p-6"><h2 className="text-lg font-display font-bold mb-1">{selectedUserTask?.tasks?.title}</h2><Badge variant="outline">{selectedUserTask?.tasks?.task_categories?.name}</Badge></div>
        <div className="bg-card border border-border rounded-2xl p-6">
          ...
          <div className="grid grid-cols-3 gap-3">
            {([{ key: "photo" as ReportFormat, icon: FileImage, label: "Фото" }, { key: "text" as ReportFormat, icon: FileText, label: "Текст" }, { key: "audio" as ReportFormat, icon: Mic, label: "Аудио" }]).map(f => (
              <button key={f.key} onClick={() => setFormat(f.key)} className={`flex flex-col items-center gap-2 p-4 rounded-xl border-2 transition-all ${format === f.key ? "border-primary bg-primary/5" : "border-border hover:border-primary/30"}`}>
        ...
        <Button className="w-full gradient-primary text-primary-foreground border-0 rounded-xl" size="lg" onClick={handleSubmit} disabled={submitting || (format === "text" && !textReport.trim())}>
